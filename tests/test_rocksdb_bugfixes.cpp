...
// TEST(RocksDB, Updates)
// {
//     RocksDBDriver rocksdb{false};
...
//     {
//         LOG(stream->get(i));
//         rocksdb.add_edge_v2(stream->get(i));
//     }
// }

TEST(RocksDB, BFSInternalImpl)
{
    CSR csr{/* directed ? */ false};
    RocksDBDriver rocksdb{/* directed ? */ false};
    ...
    LOG("Insert " << stream->num_edges() << " edges into RocksDB ...")
    uint64_t sz = stream->num_edges();
#pragma omp parallel for
    ...
    auto rocksdb_results = temp_file_path();
    LOG("RocksDB BFS: " << rocksdb_results << " ...");
    rocksdb.bfs(num_iterations, rocksdb_results.c_str());

    LOG("Validate the result ...");
    gfe::utility::GraphalyticsValidate::bfs(rocksdb_results, csr_results);
    LOG("Validation succeeded");
}

TEST(RocksDB, PageRankInternalImpl)
{
    CSR csr{/* directed ? */ false};
    RocksDBDriver rocksdb{/* directed ? */ false};
...
#pragma omp parallel for
    ...
    auto rocksdb_results = temp_file_path();
    LOG("RocksDB PageRank: " << rocksdb_results << " ...");
    rocksdb.pagerank(num_iterations, 0.85, rocksdb_results.c_str());

    LOG("Validate the result ...");
    gfe::utility::GraphalyticsValidate::pagerank(rocksdb_results, csr_results);
    LOG("Validation succeeded");
}

#else
#include <iostream>
TEST(RocksDB, Disabled)
{
    std::cout << "Tests disabled as the build does not contain the support for RocksDB.\n";
}
#endif
