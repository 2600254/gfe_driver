...
#include "third-party/neo4jdriver/api.hpp"

#define uint64_t unsigned long
namespace gfe::library {

/**
 * Wrapper to evaluate the Neo4j library
 */
class Neo4jDriver : public virtual UpdateInterface, public virtual GraphalyticsInterface {
    Neo4jDriver(const Neo4jDriver&) = delete;
    Neo4jDriver& operator=(const Neo4jDriver&) = delete;

protected:
    std::shared_ptr<neo4jDriver::Neo4j>neo4j;
    neo4jDriver::Neo4jAPI* m_pImpl; // pointer to the Neo4j handle
    ...
    uint64_t getedgeidbyIID(uint64_t sid,uint64_t did) const;
    uint64_t getedgeidbyEID(uint64_t sid,uint64_t did) const;
    double getedgebyIID(uint64_t sid,uint64_t did) const;
    double getedgebyEID(uint64_t sid,uint64_t did) const;
...
public:
     ...
     */
    Neo4jDriver(bool is_directed);

    /**
     * Destructor
     */
    virtual ~Neo4jDriver();
    uint64_t getIIDbyid(uint64_t id) const;
    uint64_t getEIDbyid(uint64_t id) const;
    uint64_t getidbyIID(uint64_t id) const;
    uint64_t getidbyEID(uint64_t id) const;
    std::shared_ptr<std::vector<std::pair<uint64_t,double>>> getedgesbyid(uint64_t id) const;
    std::shared_ptr<std::vector<std::pair<uint64_t,double>>> getedgesbyIID(uint64_t id) const;
    std::shared_ptr<std::vector<std::pair<uint64_t,double>>> getedgesbyEID(uint64_t id) const;
    /**
